...
function getInstalledVersion() {
  // Try local package.json first, fallback to node_modules
  let pkgPath = path.join(process.cwd(), 'package.json');
  if (!fs.existsSync(pkgPath)) {
    pkgPath = path.join(process.cwd(), 'node_modules', 'clawforge', 'package.json');
  }
...
/**
 * Check for a newer version (disabled for ClawForge — self-hosted, not on npm).
 */
async function runVersionCheck() {
  // Version check disabled for ClawForge — managed via git, not npm
  return;
    if (!res.ok) {
