...
async function summarizeJob(results) {
  try {
    ...
    const isFailure = ['failure', 'cancelled', 'timed_out'].includes(results.status?.toLowerCase());
    return text.trim() || (isFailure ? 'Job failed. Check run logs for details.' : 'Job finished.');
  } catch (err) {
    console.error('Failed to summarize job:', err);
    const isFailure = ['failure', 'cancelled', 'timed_out'].includes(results.status?.toLowerCase());
    return isFailure ? 'Job failed. Check run logs for details.' : 'Job finished.';
  }
}
